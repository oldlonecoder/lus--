...
std::string  alu::mNil;
#define debug_out ;
//log::debug() << log::fn::fun << "rhs=" << rhs << ", this = " << v.d << " : types: " << lexer_component::type_name(T);
...
alu alu::operator-(const alu& rhs)
{
    //log::debug() << log::fn::fun << " rhs=" << rhs.to_str();
    lrtext(rhs)
    {
...
alu alu::operator/(const alu& rhs)
{
    ...
    lr_number(rhs)
    {
        ...
        if (vr == 0.0L)
        {
            throw log::exception()[ log::except() << log::type::err  << " alu operation: " << log::code::divbyzero << " =>(" << (*this)() << " / " << rhs() << ")" ];
            return "Inf";
        }
        return {lr / vr};
    }
...
alu& alu::operator*=(const alu& rhs)
{
    ...
    lr_number(rhs)
    {
        log::debug()
            << color::orange5 << "alu"
            << color::white << "::"
            << color::white << log::fn::endl << ":" << log::fn::endl << "{" << log::fn::endl
        ...
        v.d *= rhs.v.d;
        log::write() << color::white << " = " << color::yellow << v.d  << color::white <<  log::fn::endl << '}' <<  log::fn::endl ;
        return *this;
    }
...
{
    log::debug() << log::fn::func << color::yellow << (*this)() <<color::white << ":"  ;
    lr_number(*this)
    {
        if(v.d < 0)
        {
        ...
        for (int j = 1; j <= N; j++)
            fac *= j;
        log::debug() << log::fn::func << color::yellow << fac <<color::white << ":"  ;
        return { fac };
    }
    throw log::exception()[ log::except() <<"cannot execute factorial operator on " << lexer_component::type_name(T)];
    return { false };
