...
//     if(!pksize)
//     {
//         log::write() << color::reset << "stdin triggered with 0 bytes ( likely terminal resize event )..." << log::eol;
//         return log::code::empty;
//     }
//     //log::write() << " STD INPUT EVENT  : " << color::yellow << pksize << color::reset << " bytes";
//
//
//     size_t r = ::read(STDIN_FILENO, buffer, 1024);
//
//     if(pksize>1024)
//     {
//         log::error() << color::red4 << "packet size too large: (" << color::lightsteelblue3 << pksize << color::reset << ") bytes -> possible buffer overflow (has been prevented) due to huge latency or race conditions..." << log::eol;
//         log::write() << "cancelling input event!";
//         return log::code::rejected;
//     }
//     buffer[r] = 0;
//     //log::write() << "{" << color::yellow << lus::string::bytes(buffer) << color::reset << "}";
//
...
//     {
//         if(_event_.event_type == event::type::CHARACTER)
//         {
//             log::debug() << "seq: {" << color::lightsteelblue3 << lus::string::bytes(_event_.data.kev.ansi_seq) << color::reset << "} " << log::eol;
//             if(std::strlen(_event_.data.kev.ansi_seq) > 1)
//                 log::write() << color::red4 << " -- Investigate why the sequence overflows (latency <->> async inputs) ..." << log::eol;
//         }
//
//     }
//
//     if(_event_.event_type == event::type::key_command)
//     {
//         log::write() << "KEY COMMAND: {" << color::lime << _event_.data.kev.description << color::reset << '}' << log::eol;
...
//
//         _ev_.event_type = event::type::key_command;
//         log::write() << "KEY_COMMAND: " << color::yellow << (int)*it << color::reset << "';" << log::eol;
...
//     }
//     // -- key input.
//     log::write() << "CHARACTER:'" << color::yellow << *it << color::reset << "';"  << log::eol;
//     _ev_.data.kev = key_event::nokey;
//     _ev_.event_type = event::type::CHARACTER; // Extra bytes are all ignored.
//     _ev_.data.kev.code = *it;
//     std::strncpy(_ev_.data.kev.ansi_seq, a_seq_,20);
//     return _ev_.event_type;
// }
//
//
...
//
//         if (*it >= '0' && *it <= '9') {
//             //log::write() << "cursor on digit: '" << color::yellow << (*it-'0') << color::reset << "'";
//             argument *= 10;  // NOLINT
//             argument += *it - '0';
//             continue;
//         }
...
//             }
//         }
//         log::write() << " Unterminated CSI: switchting to parse_ss_1_2" << log::eol;
//         return parse_ss_1_2(evd);
//
//         // // Invalid ESC in CSI.
//         // if (*it == '\x1B') {
...
// {
//     lus::string str = lus::string::bytes(_seq_);
