...
log::code sscan::numeric::base2()
{
    ...
    else
    {
        if(*a == '0')
        {
            ...
            {
                log::write() << " base 2 : rejected @prefix '" << color::yellow << *a << color::reset << "'.";
                return log::code::rejected;
            }
        }
    }
...
log::code sscan::numeric::base8()
{
    ...
    {
        log::write() << " base 8 :rejected @prefix '" << color::yellow << *a << color::reset << "'.";
        return log::code::rejected;
    }
...
log::code sscan::numeric::base10()
{
    ...
    //log::debug() << "decimal : a on '" << color::yellow << *a << color::reset << '\'';
    if(!std::isdigit(*a) ){
        log::write() << " base 10 :rejected @prefix '" << color::yellow << *a << color::reset << "'.";
        return log::code::rejected;
    }
    //log::debug() << "a on '" << color::yellow << *a << color::reset << '\'';
...
log::code sscan::numeric::base16()
{
    ...
    if (*a == '0')
    {
        ...
        if(std::toupper(*a) != 'x')
        {
            log::write() << " base 16 :rejected @prefix '" << color::yellow << *a << color::reset << "'.";
            return log::code::rejected;
        }
    }
    else
        if(*a !='$') // hard coded
            return log::code::rejected;
    ++a;
    log::write() << " hex prefix validated, now begin to scan : on char [" << color::yellow << *a << color::reset << "':";
    loop16:
